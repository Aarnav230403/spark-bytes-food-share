...
export default function MyActivity() {
  ...
  const fetchFollowedClubs = async (userId: string) => {
    ...
    if (error) {
      ...
      message.error("Failed to load followed clubs");
      return;
    }
    ...
    const clubsData =
      data
        ?.map((r: any) => ({
          id: r.clubs?.id,
          name: r.clubs?.name,
          category: r.clubs?.category,
          logoUrl: r.clubs?.logo_url,
        }))
        .filter(
          (c: any, i: number, arr: any[]) =>
            c.id && arr.findIndex((x) => x.id === c.id) === i
        ) || [];
  ...
  useEffect(() => {
    const fetchActivityData = async () => {
      ...
      await Promise.all([fetchEvents(user.id), fetchFollowedClubs(user.id)]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {followedClubs.map((club) => (
                  <Link key={club.id} to={`/clubs/${club.id}`}>
                    <Card className="hover:shadow-md transition-shadow cursor-pointer" hoverable>
